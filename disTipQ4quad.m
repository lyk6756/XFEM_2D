function [W,Q]=disTipQ4quad(order,phi,nodes,tip) %这里order==7,nodes是单元节点的整体坐标

epsilon = 0.00001;   %[ep‘sailEn]
corner  = [1 2 3 4 1];
...
while (inc < iter)
    ...
    x = N'*nodes(:,1);
    y = N'*nodes(:,2);  %矩形单元中心的整体坐标
    df1dr = dNdxi(:,1)' * nodes(:,1);
    df1ds = dNdxi(:,2)' * nodes(:,1);
    df2dr = dNdxi(:,1)' * nodes(:,2);
    df2ds = dNdxi(:,2)' * nodes(:,2);
 
    f1 = x - tip(1);     %Xtip=[a D/2]是裂纹尖端的整体坐标
    f2 = y - tip(2);     %单元中心和裂尖的关系

    detF = df1dr*df2ds - df1ds*df2dr ;%F矩阵的行列式

    invf(1,1) =  1.0/detF * df2ds;
    invf(1,2) = -1.0/detF * df1ds;
    invf(2,1) = -1.0/detF * df2dr;
    invf(2,2) =  1.0/detF * df1dr;   %是求F矩阵的逆,这里要将裂尖的整体坐标通过和中心整体坐标的关系转化成局部坐标
