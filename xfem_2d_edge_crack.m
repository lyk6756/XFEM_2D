...
% under remote(遥远的) traction along Y direction
...
% GET NODES ON DIRICHLET BOUNDARY AND ESSENTIAL BOUNDARY（这个看不懂）
...
t   = 1/norm(seg)*seg;    %seg最大值
for i = 1 : numnode
    x = node(i,1);
    y = node(i,2);
    l   = sqrt((x1-x0)*(x1-x0)+(y1-y0)*(y1-y0));
    phi = (y0-y1)*x + (x1-x0)*y + (x0*y1-x1*y0);
    ls(i,1) = phi/l;            % normal LS（法向和切向怎么选，水平集法的基本思想？）
...
for iel = 1 : numelem
    ...
    % ---------------------
    for kk = 1 : size(W,1)
        pt = Q(kk,:);                             % quadrature point局部坐标
        % B matrix
        [B,J0] = xfemBmatrix(pt,elemType,iel,enrich_node,xCr,xTip,alpha);%这里头有局部极坐标和局部直角坐标的转换
        % Stiffness matrix
        %ke = ke + B'*C*B*W(kk)*det(J0);
        K(sctrB,sctrB) = K(sctrB,sctrB) + B'*C*B*W(kk)*det(J0);
    end                  % end of looping on GPs
end                      % end of looping on elements  （这一部分是XFEM的核心）
...
bcwt = mean(diag(K)); % bcwt表示矩阵对角线元素的平均值
% used to keep the conditioning of the K matrix

udofs = zeros(length(dispNodes),1); %12*1的零矩阵
vdofs = zeros(length(dispNodes),1); %12*1的零矩阵

vdofs = dispNodes.*2;
udofs = [1]; % for lower left corner node

f(udofs) = 0 ;
f(vdofs) = 0 ;%左下端点x和y方向都没有位移，最下边其他点没有y方向的位移 因为已知位移都为零，可以这样设置，‘化1为0法’
              %整体刚度矩阵是对称的，稀疏带状的，而且主对角线上的元素总是正的
K(udofs,:) = 0;   % zero out the rows and columns of the K matrix
K(vdofs,:) = 0;
K(:,udofs) = 0;
K(:,vdofs) = 0;
K(udofs,udofs) = bcwt*speye(length(udofs)); % put ones*bcwt on the diagonal乘大数法
...
for e = 1 : numelem
    sctr = element(e,:);
    nn   = length(sctr);
    U     = element_disp(e,pos,enrich_node,u); %从位移列中选出一个单元内每个节点以及虚拟节点的位移
    for q=1:nn
        pt = stressPoints(q,:);   
...
for iel = 1 : size(Jdomain,2) %1：22
    ...
    for q = 1:size(W,1)
        ...
        % +++++++++++++++++++++++++ 
        % Gradient of displacement(位移梯度)
        % +++++++++++++++++++++++++ 
        % need to compute u,x u,y v,x v,y, stored in matrix H
        [B,J0] = xfemBmatrix(pt,elemType,e,enrich_node,xCr,xTip,alpha);% 单元循环内嵌套高斯点的循环
        leB = size(B,2);   %单元B矩阵的列数
        
        % nodal displacement of current element
        % taken from the total nodal parameters u
        U = element_disp(e,pos,enrich_node,u);  %单元节点位移
        ...
        % Gradient of q
        % ++++++++++++++        
        q     = qnode(iel,:);  %这22个单元做循环，和原先的陈述是一样的，即圆内节点q值为1，圆外节点q值为0
        gradq = q*dNdx; %1*2
        ...
        gradqloc    = QT*gradq';
        graddisploc = QT*H*QT';
        stressloc   = QT*sigma(voit2ind)*QT';%转换成当前倾角下的分量
