...
% Solving problem: Edged crack with exact displacement as imposed boundary
% condition
...
disp('No applied force in this problem')
% -------------------------------------
...
ndisn = length(dispNodes); % number of constrained nodes
...
for i = 1 : ndisn 
    x = node(dispNodes(i),:) ;
    [ux,uy] = exact_Griffith(x,E,nu,stressState,sigmato,xTip,seg,cracklength) ;
    uFixed(i) = ux ;
    vFixed(i) = uy ;
end
...
% put ones*bcwt on the diagonal
K(udofs,udofs) = bcwt*speye(ndisn); 
K(vdofs,vdofs) = bcwt*speye(ndisn);
...
hold on
fac = 30;
plot_mesh(node+fac*[ux_exact' uy_exact'],element,elemType,'b-');
title(' Exact deformed configuration ')
% -------------------------------------------------------------------------
plot(node(dispNodes,1)+fac*uFixed',node(dispNodes,2)+fac*vFixed','b*');
plot(node(dispNodes,1)+fac*u(udofs),node(dispNodes,2)+fac*u(vdofs),'rs');
% Plot two deformed configurations for comparison
figure
hold on
plot_mesh(node+fac*[ux_exact' uy_exact'],element,elemType,'b-');
